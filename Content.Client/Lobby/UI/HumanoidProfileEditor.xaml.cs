...
namespace Content.Client.Lobby.UI
{
    ...
    public sealed partial class HumanoidProfileEditor : BoxContainer, ILinkClickHandler
    {
        ...
        private BackgroundCategory _currentBackgroundCategory = BackgroundCategory.Nationality;
        private const int BackgroundButtonWrap = 18;
            ...
            MarkingManager markings)
        {
            ...
            BackgroundCategoryNationality.Text = WrapButtonText(Loc.GetString("humanoid-profile-editor-nationality-label"), BackgroundButtonWrap);
            BackgroundCategoryEmployer.Text = WrapButtonText(Loc.GetString("humanoid-profile-editor-employer-label"), BackgroundButtonWrap);
            BackgroundCategoryLifepath.Text = WrapButtonText(Loc.GetString("humanoid-profile-editor-lifepath-label"), BackgroundButtonWrap);
            ...
            BackgroundCategoryNationality.OnPressed += _ =>
            {
                SetBackgroundCategory(BackgroundCategory.Nationality);
            };
            BackgroundCategoryEmployer.OnPressed += _ =>
            {
                SetBackgroundCategory(BackgroundCategory.Employer);
            };
            BackgroundCategoryLifepath.OnPressed += _ =>
            {
                SetBackgroundCategory(BackgroundCategory.Lifepath);
            };
        ...
        private void RefreshNationalities()
        {
            ...
            _nationalities.AddRange(_prototypeManager.EnumeratePrototypes<NationalityPrototype>().OrderBy(n => n.SortOrder));
            if (Profile != null && !_nationalities.Any(n => n.ID == Profile.Nationality) && _nationalities.Count > 0)
            {
                SetNationality(_nationalities[0].ID);
            }
        }

        private void RefreshLifepaths()
        {
            _lifepaths.Clear();

            _lifepaths.AddRange(_prototypeManager.EnumeratePrototypes<LifepathPrototype>().OrderBy(l => Loc.GetString(l.NameKey)));
            if (Profile != null && !_lifepaths.Any(l => l.ID == Profile.Lifepath) && _lifepaths.Count > 0)
            {
                SetLifepath(_lifepaths[0].ID);
            }
        }
        ...
        private void RefreshEmployers()
        {
            ...
            _employers.AddRange(_prototypeManager.EnumeratePrototypes<EmployerPrototype>().OrderBy(e => Loc.GetString(e.NameKey)));
            if (Profile != null && !_employers.Any(e => e.ID == Profile.Employer) && _employers.Count > 0)
            {
                SetEmployer(_employers[0].ID);
            }
        }
        ...
        {
            Profile = Profile?.WithLifepath(lifepath);
            SetDirty();
            RefreshJobs();
        ...
        private void SetBackgroundCategory(BackgroundCategory category)
        {
            ...
            BackgroundCategoryNationality.Pressed = category == BackgroundCategory.Nationality;
            BackgroundCategoryEmployer.Pressed = category == BackgroundCategory.Employer;
            BackgroundCategoryLifepath.Pressed = category == BackgroundCategory.Lifepath;
            RebuildBackgroundOptions();
            UpdateBackgroundDescription();
        }
        ...
        private void RebuildBackgroundOptions()
        {
            ...
            {
                case BackgroundCategory.Nationality:
                    foreach (var proto in _nationalities)
                    {
                        var option = new Button
                        {
                            Text = WrapButtonText(Loc.GetString(proto.NameKey), BackgroundButtonWrap),
                            ToggleMode = true,
                            Pressed = Profile?.Nationality == proto.ID,
                        ...
                        option.OnPressed += _ =>
                        {
                            SetNationality(proto.ID);
                            SetBackgroundCategory(BackgroundCategory.Nationality);
                        };
                        BackgroundOptions.AddChild(option);
                    }
                    break;
                case BackgroundCategory.Employer:
                    foreach (var proto in _employers)
                    {
                        var option = new Button
                        {
                            Text = WrapButtonText(Loc.GetString(proto.NameKey), BackgroundButtonWrap),
                            ToggleMode = true,
                            Pressed = Profile?.Employer == proto.ID,
                            HorizontalExpand = true,
                            ClipText = false,
                            MinSize = new Vector2(0, 40),
                            TextAlign = Label.AlignMode.Left
                        ...
                        option.OnPressed += _ =>
                        {
                            SetEmployer(proto.ID);
                            SetBackgroundCategory(BackgroundCategory.Employer);
                        };
                        BackgroundOptions.AddChild(option);
                    }
                    break;
                case BackgroundCategory.Lifepath:
                    foreach (var proto in _lifepaths)
                    {
                        var option = new Button
                        {
                            Text = WrapButtonText(Loc.GetString(proto.NameKey), BackgroundButtonWrap),
                            ToggleMode = true,
                            Pressed = Profile?.Lifepath == proto.ID,
                            HorizontalExpand = true,
                            ClipText = false,
                            MinSize = new Vector2(0, 40),
                            TextAlign = Label.AlignMode.Left
        ...
        private void UpdateBackgroundDescription()
        {
            switch (_currentBackgroundCategory)
            {
                case BackgroundCategory.Nationality when Profile != null:
                    if (_prototypeManager.TryIndex<NationalityPrototype>(Profile.Nationality, out var natProto))
                    {
                    ...
                    break;
                case BackgroundCategory.Employer when Profile != null:
                    if (_prototypeManager.TryIndex<EmployerPrototype>(Profile.Employer, out var empProto))
                    {
                        ...
                        if (Profile.Employer == "AlphaCentauriAb")
                        {
                    ...
                    break;
                case BackgroundCategory.Lifepath when Profile != null:
                    if (_prototypeManager.TryIndex<LifepathPrototype>(Profile.Lifepath, out var lifeProto))
                    {
                        ...
                        if (Profile.Lifepath == "ColonialEmbryo")
                        {
