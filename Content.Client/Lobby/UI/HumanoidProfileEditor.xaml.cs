...
namespace Content.Client.Lobby.UI
{
    ...
    public sealed partial class HumanoidProfileEditor : BoxContainer, ILinkClickHandler
    {
        ...
        private void RefreshNationalities()
        {
            ...
            {
                var proto = _nationalities[i];
                NationalityButton.AddItem(Loc.GetString(proto.NameKey), i);
                if (Profile?.Nationality == proto.ID)
                    NationalityButton.SelectId(i);
            }

            if (Profile != null && !_nationalities.Any(n => n.ID == Profile.Nationality) && _nationalities.Count > 0)
            {
                NationalityButton.SelectId(0);
        ...
        private void RefreshEmployers()
        {
            ...
            {
                var proto = _employers[i];
                EmployerButton.AddItem(Loc.GetString(proto.NameKey), i);
                if (Profile?.Employer == proto.ID)
                    EmployerButton.SelectId(i);
            }

            if (Profile != null && !_employers.Any(e => e.ID == Profile.Employer) && _employers.Count > 0)
            {
                EmployerButton.SelectId(0);
        ...
        private void UpdateNationalityDescription()
        {
            if (_nationalities.Count == 0)
            {
                NationalityDescriptionLabel.SetMessage(FormattedMessage.Empty);
                return;
            }

            var selected = Math.Clamp(NationalityButton.SelectedId, 0, _nationalities.Count - 1);
            var proto = _nationalities[selected];
            NationalityDescriptionLabel.SetMessage(FormattedMessage.FromMarkup(Loc.GetString(proto.DescriptionKey)));
        }

        private void UpdateEmployerDescription()
        {
            if (_employers.Count == 0)
            {
                EmployerDescriptionLabel.SetMessage(FormattedMessage.Empty);
                return;
            }

            var selected = Math.Clamp(EmployerButton.SelectedId, 0, _employers.Count - 1);
            var proto = _employers[selected];
            EmployerDescriptionLabel.SetMessage(FormattedMessage.FromMarkup(Loc.GetString(proto.DescriptionKey)));
        }
