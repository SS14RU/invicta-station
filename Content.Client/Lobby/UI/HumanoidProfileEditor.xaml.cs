...
namespace Content.Client.Lobby.UI
{
    ...
    public sealed partial class HumanoidProfileEditor : BoxContainer, ILinkClickHandler
    {
        ...
        private List<CitizenshipPrototype> _citizenships = new();
        private List<BackgroundPrototype> _backgrounds = new();
        private List<PlanetPrototype> _planets = new();
        private enum BackgroundCategory
        {
            Citizenship,
            Planet,
            Background
            ...
            MarkingManager markings)
        {
            ...
            TabContainer.SetTabTitle(1, Loc.GetString("humanoid-profile-editor-background-tab"));
            ...
            RefreshBackgrounds();
            RefreshPlanets();
            RebuildBackgroundOptions();

            BackgroundCategoryCitizenship.Text = WrapButtonText(Loc.GetString("humanoid-profile-editor-citizenship-label"), BackgroundButtonWrap);
            BackgroundCategoryPlanet.Text = WrapButtonText(Loc.GetString("humanoid-profile-editor-planet-label"), BackgroundButtonWrap);
            BackgroundCategoryBackground.Text = WrapButtonText(Loc.GetString("humanoid-profile-editor-background-label"), BackgroundButtonWrap);

            BackgroundTitleLabel.FontColorShadowOverride = Color.Black;
            BackgroundTitleLabel.ShadowOffsetXOverride = 1;
            BackgroundTitleLabel.ShadowOffsetYOverride = 1;

            BackgroundCategoryCitizenship.OnPressed += _ =>
            {
                SetBackgroundCategory(BackgroundCategory.Citizenship);
            };
            BackgroundCategoryPlanet.OnPressed += _ =>
            {
                SetBackgroundCategory(BackgroundCategory.Planet);
            };
            BackgroundCategoryBackground.OnPressed += _ =>
            {
                SetBackgroundCategory(BackgroundCategory.Background);
            };

            BackgroundToggleTextButton.OnPressed += _ =>
            {
            ...
            TabContainer.SetTabTitle(4, Loc.GetString("humanoid-profile-editor-traits-tab")); // CorvaxGoob-TTS-Edit
        ...
        {
            var index = 0;
            TabContainer.SetTabTitle(index++, Loc.GetString("humanoid-profile-editor-appearance-tab"));

            if (_ttsTab != null)
                TabContainer.SetTabTitle(index++, Loc.GetString("humanoid-profile-editor-voice-tab"));

            TabContainer.SetTabTitle(index++, Loc.GetString("humanoid-profile-editor-background-tab"));
            TabContainer.SetTabTitle(index++, Loc.GetString("humanoid-profile-editor-jobs-tab"));
            TabContainer.SetTabTitle(index++, Loc.GetString("humanoid-profile-editor-antags-tab"));
            TabContainer.SetTabTitle(index++, Loc.GetString("humanoid-profile-editor-traits-tab")); // CorvaxGoob-TTS-Edit
            TabContainer.SetTabTitle(index++, Loc.GetString("humanoid-profile-editor-markings-tab"));

            if (_flavorText != null)
                TabContainer.SetTabTitle(index, Loc.GetString("humanoid-profile-editor-flavortext-tab"));
        }
        ...
        private void RefreshCitizenships()
        {
            ...
            _citizenships.AddRange(_prototypeManager.EnumeratePrototypes<CitizenshipPrototype>().OrderBy(n => n.SortOrder));
            if (Profile != null && !_citizenships.Any(n => n.ID == Profile.Citizenship) && _citizenships.Count > 0)
            {
                SetCitizenship(_citizenships[0].ID);
            }
        }
        ...
        public void RefreshJobs()
        {
            ...
            var profile = Profile ?? (HumanoidCharacterProfile?) _preferencesManager.Preferences?.SelectedCharacter;
        ...
        private void SetCitizenship(string citizenship)
        {
            ...
            RefreshJobs();
            EnsureBackgroundConsistency();
            if (_currentBackgroundCategory == BackgroundCategory.Citizenship)
        ...
        {
            Profile = Profile?.WithBackground(background);
            SetDirty();
            RefreshJobs();
            EnsureBackgroundConsistency();
            if (_currentBackgroundCategory == BackgroundCategory.Background)
                UpdateBackgroundDescription();
            UpdateBackgroundSummary();
        }
        ...
        {
            Profile = Profile?.WithPlanet(planet);
            SetDirty();
            RefreshJobs();
            EnsureBackgroundConsistency();
            if (_currentBackgroundCategory == BackgroundCategory.Planet)
                UpdateBackgroundDescription();
            UpdateBackgroundSummary();
        }
        ...
        private void SetBackgroundCategory(BackgroundCategory category)
        {
            _currentBackgroundCategory = category;
            BackgroundCategoryCitizenship.Pressed = category == BackgroundCategory.Citizenship;
            BackgroundCategoryPlanet.Pressed = category == BackgroundCategory.Planet;
            BackgroundCategoryBackground.Pressed = category == BackgroundCategory.Background;
            RebuildBackgroundOptions();
            UpdateBackgroundDescription();
        }

        private void RebuildBackgroundOptions()
        {
            BackgroundOptions.DisposeAllChildren();

            var validPlanets = GetValidPlanets();
            var validBackgrounds = GetValidBackgrounds();

            switch (_currentBackgroundCategory)
            {
                case BackgroundCategory.Citizenship:
                    foreach (var proto in _citizenships)
                    {
                        ...
                        option.OnPressed += _ =>
                        {
                            SetCitizenship(proto.ID);
                            SetBackgroundCategory(BackgroundCategory.Citizenship);
                        };
                        BackgroundOptions.AddChild(option);
                    }
                    break;
                case BackgroundCategory.Planet:
                    foreach (var proto in validPlanets)
                    {
                        var option = new Button
                        {
                            Text = WrapButtonText(Loc.GetString(proto.Name), BackgroundButtonWrap),
                    ...
                    break;
                case BackgroundCategory.Background:
                    foreach (var proto in validBackgrounds)
                    {
                        var option = new Button
                        {
                            Text = WrapButtonText(Loc.GetString(proto.Name), BackgroundButtonWrap),
        ...
        private List<PlanetPrototype> GetValidPlanets()
        {
            if (Profile == null)
                return _planets;

            var citizenship = Profile.Citizenship;
            // Empty AllowedCitizenships = no restriction.
            return _planets
                .Where(emp =>
                    emp.AllowedCitizenships.Count == 0 ||
                    emp.AllowedCitizenships.Contains(citizenship))
                .ToList();
        }

        private List<BackgroundPrototype> GetValidBackgrounds()
        {
            if (Profile == null)
                return _backgrounds;

            var planet = Profile.Planet;
            // Empty AllowedPlanets = no restriction.
            return _backgrounds
                .Where(life =>
                    life.AllowedPlanets.Count == 0 ||
                    life.AllowedPlanets.Contains(planet))
                .ToList();
        }

        private void UpdateBackgroundDescription()
        {
            switch (_currentBackgroundCategory)
            {
        ...
        private void EnsureBackgroundConsistency()
        {
            if (_ensuringBackground || Profile == null)
                return;

            _ensuringBackground = true;
            try
            {
                var validPlanets = GetValidPlanets();
                if (validPlanets.Count == 0 && _planets.Count > 0)
                {
                    Profile = Profile.WithPlanet(_planets[0].ID);
                    SetDirty();
                }
                else if (validPlanets.Count > 0 && !validPlanets.Any(e => e.ID == Profile.Planet))
                {
                    Profile = Profile.WithPlanet(validPlanets[0].ID);
                    SetDirty();
                }

                var validBackgrounds = GetValidBackgrounds();
                if (validBackgrounds.Count == 0 && _backgrounds.Count > 0)
                {
                    Profile = Profile.WithBackground(_backgrounds[0].ID);
                    SetDirty();
                }
                else if (validBackgrounds.Count > 0 && !validBackgrounds.Any(l => l.ID == Profile.Background))
                {
                    Profile = Profile.WithBackground(validBackgrounds[0].ID);
                    SetDirty();
                }
        ...
        private void UpdateBackgroundSummary()
        {
            ...
            BackgroundSummaryBackgroundValue.Text = backgroundText;
        }
        ...
        private static string WrapButtonText(string text, int maxCharsPerLine)
        {
            ...
            {
                var word = words[i];
                var addSpace = currentLineLength > 0 ? 1 : 0;
                if (currentLineLength + addSpace + word.Length > maxCharsPerLine)
                {
                    builder.Append('\n');
                    builder.Append(word);
        ...
        private void ToggleBackgroundText()
        {
            _backgroundTextHidden = !_backgroundTextHidden;
            UpdateBackgroundTextVisibility();
        }

        private void UpdateBackgroundTextVisibility()
        {
            var visible = !_backgroundTextHidden;
            BackgroundTitleLabel.Visible = visible;
            BackgroundDescriptionLabel.Visible = visible;
            BackgroundDimmer.Visible = visible;
            BackgroundToggleTextButton.Text = Loc.GetString(visible
                ? "humanoid-profile-editor-toggle-background-text-hide"
                : "humanoid-profile-editor-toggle-background-text-show");
        }

        private void UpdateBackgroundImage(string? texturePath)
        {
            ...
            var texture = _resCache.GetResource<TextureResource>(texturePath);
        ...
        // INV: Open guidebook entries directly from background/citizenship text links.
