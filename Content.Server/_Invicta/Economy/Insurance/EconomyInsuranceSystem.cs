...
using Content.Shared._Invicta.Economy.Insurance;
...
namespace Content.Server._Invicta.Economy.Insurance;
...
public sealed class EconomyInsuranceSystem : EconomyInsuranceSystemShared
{
    ...
    private PreparedInsurerData? PerformPrepareData(EntityUid playerUid, HumanoidCharacterProfile profile, ProtoId<JobPrototype>? job)
    {
        ...
        // dnaComponent.DNA can be null if no DNA recorded; fall back to empty string to satisfy non-null contract.
        return new(cardUid.Value, insurance, insurance, cardComponent.FullName, accountHolderComponent.AccountID, dnaComponent.DNA ?? string.Empty);
    }
    ...
    {
        // Profile no longer stores insurance; fall back to default matching.
        return MatchDefaultInsuranceProto(job);
    }
